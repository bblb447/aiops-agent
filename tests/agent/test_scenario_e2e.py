...
脚本化模型按计划依次产出工具调用（query_metric -> search_runbook -> final_answer），
证明 Alert(Incident) -> Agent -> Tools -> RCA 全链路可用。
"""
import json
...
def test_e2e_cpu_gc_release_scenario(monkeypatch, tmp_path):
    ...
    plan = [
        ...
        {"name": "final_answer", "arguments": {
            "answer": "根因: 内存泄漏导致 CPU 持续走高，置信度 0.85；证据: cpu_usage=95.2，命中内存泄漏 runbook"
        }},
    ]
    ...
    assert "内存泄漏" in got.root_cause
    assert out == got.root_cause
